...
It is not trying to beat a production inference server. This is a learning project: the point is to build the real pieces — a request queue, dynamic batching, quantization, GPU memory management — one layer at a time, and actually understand each one instead of importing it.
...
- **Understand before optimizing.** Each piece is written by hand before reaching for a library that hides it. The batching loop, the queue, the metrics — all visible in one file.
...
- **Observability** — Prometheus metrics and Grafana dashboards instead of hand-rolled counters
...
- [api/api_server.py](api/api_server.py) — the whole server: endpoints, request queue, batching worker, and metrics
- [experiments/benchmark.py](experiments/benchmark.py) — async load tester
- [requirements.txt](requirements.txt) — dependencies

## License

Not yet licensed — this is a personal learning project. Reach out if you want to use any of it.
